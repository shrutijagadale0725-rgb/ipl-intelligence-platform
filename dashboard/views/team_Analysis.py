...
# ═════════════════════════════════════════════════════════════
# PAGE-SPECIFIC HTML HELPERS (kept here because they're only used
# on the Team Analysis page — shared ones now live in shared_styles.py)
# ═════════════════════════════════════════════════════════════
...
def compute_rolling_form(team, matches_df, season=None):
    ...
    for _, row in team_matches.tail(5).iterrows():
        results.append(1 if row['winner'] == team else 0)
...
def compute_toss_intelligence(matches_df):
    ...
    toss_intel = toss_intel[(bat_wins['total'] + field_wins['total']) >= 10]
...
def show_team_analysis():
    ...
    st.markdown(section_label("📊 Current Form & Overall Performance", "How the team has performed recently vs historically."), unsafe_allow_html=True)
    ...
    with col1:
        ...
        st.markdown(takeaway(f"{sel_team} has a {win_rate:.0f}% historical win rate across {total_matches} matches."), unsafe_allow_html=True)
    ...
    with col2:
        ...
        st.markdown(takeaway(f"Recent form: {win_pct*100:.0f}% win rate in last {m_played} games.") if m_played > 0 else "", unsafe_allow_html=True)
    ...
    st.markdown(section_label("🏟️ Venue Strength Matrix", "Win percentage at each ground (min. 5 matches)."), unsafe_allow_html=True)
    ...
    if venue_data:
        ...
        with vc1:
            if best_venues:
                ...
                st.markdown(takeaway(f"Dominates at venues like {best_venues[0][0]} with {best_venues[0][1]*100:.0f}% win rate."), unsafe_allow_html=True)
        with vc2:
            if worst_venues:
                st.markdown(
                    f'<div style="background:#0E1420;border:1px solid rgba(255,255,255,0.07);border-radius:16px;padding:16px 20px;">'
                ...
                st.markdown(takeaway(f"Struggles at {worst_venues[0][0]} (win rate drops to {worst_venues[0][1]*100:.0f}%)."), unsafe_allow_html=True)
    else:
        st.info("Insufficient venue data to generate matrix.")

    # ── SECTION 3: TOSS DECISION INTELLIGENCE ─────────────
    st.markdown(section_label("🪙 Toss Decision Intelligence", "Where does winning the toss actually matter?"), unsafe_allow_html=True)
    ...
    if toss_data:
        ...
        st.markdown(takeaway("At these venues, the toss winner's decision to bat or field first shifts win probability by >15%."), unsafe_allow_html=True)
    ...
    st.markdown(section_label("⚔️ Head-to-Head Comparison", "Select opponent to see matchup history."), unsafe_allow_html=True)
    ...
    if len(h2h_data) > 0:
        ...
        if diff > 2:
            st.markdown(takeaway(f"{dominance} dominates this rivalry, winning {diff} more matches."), unsafe_allow_html=True)
        else:
            st.markdown(takeaway("Highly competitive rivalry with a narrow historical margin."), unsafe_allow_html=True)
    else:
